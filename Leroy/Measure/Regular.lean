...
/-
'norm_add_mul_self_real' depends on axioms:

* Quot.sound: norm_add_mul_self_real → NormedAddCommGroup → NormedAddCommGroup.mk → Norm → Norm.mk →
Real → Real.ofCauchy → CauSeq.Completion.Cauchy → StrictOrderedCommSemiring.toOrderedCommSemiring →
StrictOrderedSemiring.toOrderedSemiring → StrictOrderedSemiring.toOrderedSemiring.proof_1 →
Classical.decRel → Classical.propDecidable → Classical.propDecidable.proof_1 → Classical.em →
funext → Quot.sound

* Classical.choice: norm_add_mul_self_real → NormedAddCommGroup → NormedAddCommGroup.mk → Norm →
Norm.mk → Real → Real.ofCauchy → CauSeq.Completion.Cauchy →
StrictOrderedCommSemiring.toOrderedCommSemiring → StrictOrderedSemiring.toOrderedSemiring →
StrictOrderedSemiring.toOrderedSemiring.proof_1 → Classical.decRel → Classical.propDecidable →
Classical.choice

* propext: norm_add_mul_self_real → NormedAddCommGroup → NormedAddCommGroup.mk → Norm → Norm.mk →
Real → Real.ofCauchy → CauSeq.Completion.Cauchy → LinearOrderedField → LinearOrderedField.mk →
RatCast → RatCast.mk → Rat → Rat.mk' → Nat.Coprime → Nat.gcd → Nat.gcd._unary →
Nat.gcd._unary.proof_2 → Init.Core._auxLemma.7 → propext
-/
end
