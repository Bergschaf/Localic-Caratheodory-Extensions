    ...
    have h14 : sSup ((Set.range fun b => sSup (Set.range fun (h : b ∈ Subtype.val '' s) => ↑a ⊓ b))) ≤ sSup ((Subtype.val '' Set.range fun b => e_schlange (sSup (Subtype.val '' Set.range fun (h : b∈ s) => e_schlange (↑a ⊓ ↑b))))) := by
        ...
        exact
          h_e_image img (↑(a ⊓ ⟨a1, ha1⟩))
            (h_e_image1 (↑(a ⊓ ⟨a1, ha1⟩)) (congrArg e.obj (a_inf_b_mem ⟨a1, ha1⟩)))
...
lemma nucleus_subframe (n : Nucleus E) : (∃ h : Order.Frame (Image n.toFun), ∃ f : FrameHom E X, n.toFun = ((f_obenstern f) ⋙ (f_untenstern f)).obj ∧ ∃ k : Leroy_Embedding f, true) :=  by
  sorry
...
#print axioms nucleus_equiv_subframe_1
...
/-lemma nucleus_equiv_subframe_3' (e : E ⥤ E) : [Nucleus e] → (∃ h : Order.Frame (Image e), ∃ f : FrameHom E (Image e), e = (f_obenstern f) ⋙ (f_untenstern f) ∧ ∃ _ : Leroy_Embedding f, true)  := by
  intro n
  let img := Image e
  ...
  let frameHom : FrameHom E E := ⟨⟨⟨e_schlange, e_schlange_preserves_inf⟩, (by simp[e_schlange,top];exact rfl)⟩, aux43⟩
  use frameHom
  apply And.intro
  . have h : ∀ x : img, (f_untenstern frameHom).obj x = x := by
      simp [f_untenstern, frameHom, e_schlange]
