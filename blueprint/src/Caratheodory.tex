...
\begin{definition}[Measure on Locales]
...
\begin{lemma}[Caratheodory Extensions are monotonic]
    \label{lem:monotonic}
    \uses{def:measure_on_locals,def:caratheodory}
    \lean{Measure.caratheodory.mono}
    \leanok
...
(Leroy lemme 3.2)
    ...
    \lean{Sublocale.intersection_Open_Neighbourhhood}
    \leanok
...
\begin{proposition}[reductive]
    ...
    \uses{def:sublocal,lem:regularity_of_sublocals,lem:restriction_to_sublocale}
    For any measure on a local $X$, the caratheodory extension is
...
(Leroy Lemme 3.6)
    ...
    \lean{Measure.caratheodordy.preserves_iInf}
    \leanok
For any measure on a local $X$ and a decreasing family $(V_i)_{i\in I}$ of open sublocals, the caratheodory extension fulfills: $\mu (\inf_{i\in I} V_i) = \inf_{i\in I} \mu(V_i)$.
\end{lemma}
\begin{proof}
    \leanok
\end{proof}
...
(Leroy lemme 3.7 et principal)
    ...
    \uses{prop:reductive,lem:commutes_with_inf_opens,lem:regularity_of_sublocals}
    For any measure on a local $X$, the caratheodory extension is
    regular $\mu (\inf_{i\in I} A_i) = \inf{i\in I} \mu(A_i)$. For decreasing families $(A_i)_{i\in I}$
\end{proposition}
\begin{proof}
    \leanok
\end{proof}
...
\begin{theorem}[Main Theorem (very important)]
    ...
    For any measure on a local $X$, the caratheodory extension is
    \begin{enumerate}
        \item strictly additiv, i.e. $\mu (A \cup B) = \mu(A) + \mu(B) - \mu(A \cap B)$ for all $A,B\in X$,
        \item commutes with inf $\mu (\inf_{i\in\mathbb{N}} A_i) = \inf_{i\in\mathbb{N}} \mu(A_i)$ for a familiy $(A_i)_{i\in\mathbb{N}}$ of elements $A_i\in X$,
