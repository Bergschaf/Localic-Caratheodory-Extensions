...
\newcommand{\leanok}[1]{%
    ...
    \hyperlink{Lean:#1}{\textcolor{darkgreen}{\textbf{[Lean: #1]}}}%
    % If the key is not already in \lean@keys, then add it and append the entry:
    \ifinlist{#1}{\leankeys}{}{%
         \listgadd{\leankeys}{#1}%
         \gappto{\leanentries}{%
             \item \hypertarget{Lean:#1}{} \textbf{#1}\quad\url{https://bergschaf.github.io/Localic-Caratheodory-Extensions/docs/find/\hash doc/#1}%
         }%
    }%
}
...
\newcommand{\mathlibok}[1]{%
    ...
    \ifinlist{#1}{\mathlibkeys}{}{%
         ...
         \gappto{\mathlibentries}{%
             \item \hypertarget{Lean:#1}{} \textbf{#1}\quad\url{https://leanprover-community.github.io/mathlib4_docs/find/\hash doc/#1}%
         }%
    }%
}
...
% \includegraphics[scale=0.36]{OIP.jpg}
    ...
    \begin{definition}
        ...
        \end{itemize}
    \end{definition}
    \noindent Da wir es nun mit einem äußeren Maß statt einem Maß auf der Potenzmenge von $\mathbb{R}^n$ zu tun haben,
    gelten Eigenschaften wie bspw. die strikte Additivität ($\mu (A\cup B)=\mu (A)+\mu(B)-\mu(A\cap B)$), welche man bei
    Maßen hat, allgemein nicht mehr. Die Folgen spiegeln sich unter anderem im Banach-Tarski-Paradoxon wider; eine Kugel
    kann ausgehend von einer komplexen Zerlegung verdoppelt werden \autocite{noauthor_banach-tarski-paradoxon_2024}. Um solche Kuriositäten zu umgehen, schränkt
    man sich üblicherweise auch beim äußeren Maß auf sog. messbare Mengen bzgl. dieses äußeren Maßes ein. Eine Menge $A$
    heißt messbar bzgl. einem äußeren Maß $\mu^*$, wenn sie die Caratheodory Bedingung $\mu^*(E)=\mu^*(A\cap E)+\mu^*(A^c\cap E)$ ($E\subset X$ beliebig) erfüllt. Unserer Meinung nach ist diese Einschränkung jedoch nicht besonders elegant, da wir erneut Mengen haben, denen wir keine sinnvolle Größe zuordnen können. Zu diesen Mengen gehören insbesondere auch die Banach-Tarski-Mengen. Statt die Fortsetzung eines Maßes über die klassische Mengenlehre zu betrachten und sich dann auf messbare Mengen zu beschränken, wollen wir das Problem anders lösen. Die zugrundeliegende Idee ist dabei, das äußere Maß auf der Potenzmenge der zugrundeliegenden Menge zu einem Maß zu machen. Entsprechend benötigen wir eine bessere Definition von Teilmengen, Schnitten, Vereinigungen etc.. Eine Lösung für unser Ziel bietet dabei die Theorie der Lokale.
    ...
    vollständigen Heyting Algebra) erfüllt. (TODO Mathlib Quelle).
    \begin{definition}
        Ein Lokal $E$ besteht aus $(O(E), \le)$, wobei $O(E)$ eine teilgeordnete Menge mit der Ordnuungsrelation $\le$
    ...
    Zunächst werden wir zeigen, dass die Nuklei $N(E)$ auf der Lokale $E$ einen Frame \autocite{noauthor_frame_nodate} bilden. Da die
    ...
    Die Nuklei wurden nicht nur bei Leroy verwendet, sie
    ...
    \end{definition} TODO hier überall Lean häkchen
    Die Nuklei bilden damit eine Halbordnung. \\
    Zusätzlich dazu benötigt die Frame Struktur der Nuklei zwei besondere Nuklei; den größten und den kleisten Nukleus:
    \begin{definition}
        Der größte Nukleus $\top_n$ ist definiert durch $\top_n(a)=\top_E$ für alle $a\in O(E)$.\\
        Der kleinste Nukleus $\bot_n$ ist definiert durch $\bot_n(a)= a$ für alle $a \in O(E)$
    \end{definition}
    ...
    Bei beliebigen Infima von Nuklei können wir uns an der Definition von Peter Johnstone in seinem Buch \glqq Stone spaces \grqq (TODO Quelle) orientieren:
    \begin{definition}
        Das Infimum einer Menge $S$ an Nuklei auf $E$ bildet einen Nukleus mit folgender Funktion:
        \[(\bigwedge_n S)(a) = \bigwedge_E \{j(a) \| j \in S\}\]
        für alle $a \in O(E)$
    \end{definition}
    Wir haben in Lean verifiziert, dass diese Definition tatsächlich ein Nukleus ist und die Bedingungen eines Infimums erfüllt. \\
    ...
    \begin{definition}
        ...
        \[(\bigvee_n S)(a) = (\bigwedge_n \{w \in N(E) \| x \le w\})(a)\]
        für alle $x \in S$.
    \end{definition}
    Diese Defintion ist ebenfalls ein Nukleus und erfüllt definitionsgemäß die Eigenschaften eines Supremums. \\
    Damit haben wir gezeigt, dass die Nuklei $N(E)$ einer Lokale $E$ einen Vollständigen Verband bilden.
    Um nun zu zeigen, dass die Nuklei einen Frame bilden, reicht es aus, zu zeigen, dass die Nuklei eine Heyting Algebra (TODO QUelle) bilden, da jeder Vollständige Verband, der eine Heyting Algebra ist, einen Frame bildet (TODO Mathlib Quelle).
    ...
    \begin{definition}
        ...
        also $n_Y(V) \le n_X(V)$ für alle $V\in O(E)$
    \end{definition}
    ...
    Aufgrund der Dualen Ordnung der Nuklei können wir von der Frame-Struktur der Nuklei auf die Coframe Struktur (
    TODO Mathlib Quelle) der Unterlokalen schließen und damit die Interaktionen von Unterlokalen definieren. \\
    Wir wollen uns nun aber noch etwas genauer mit der Definition der Vereinigung von Unterlokalen beschäftigen, da
    sie einen Unterschied zwischen der Arbeit von Johnstone und Leroy darstellt: TODO
...
% TODO Quelle leroy seite 5
    ...
    \begin{proof}
        ...
        Wir wissen, dass $c \le c \Rightarrow a$ genau dann, wenn $c \cap v \le a$, daher sind alle Elemente $S$ kleiner oder gleich wie $v \Rightarrow a$.
    \end{proof}
